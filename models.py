...
class StartupState(BaseModel):
    ...
    churn_rate: float = Field(default=0.03, ge=0.0, le=1.0, description="Fraction of users lost per step")
    product_quality: float = Field(default=0.55, ge=0.0, le=1.0, description="Product quality score")
    market_demand: float = Field(default=0.6, ge=0.0, le=1.0, description="External market demand score")
